...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        self.eval = torch.nn.Linear(config.hidden_size, len(list(self.bert.parameters())[-1]))
        ...
        self.clf_layer = torch.nn.Linear(len(list(self.bert.parameters())[-1]), self.num_labels)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        perturbed_embeddings = embeddings + (torch.randn(size = embeddings.shape) * 1e-6)

        output = self.bert(input_ids, attention_mask)['pooler_output']
        output = self.dropout(output)
        logits =  self.clf_layer(output)
        logits = F.log_softmax(logits, dim = -1)

        perturbed_outputs = self.bert(None, None, perturbed_embeddings)['pooler_output']
        perturbed_outputs = self.dropout(perturbed_outputs)
        perturbed_logits =  self.clf_layer(perturbed_outputs)
        perturbed_logits = F.log_softmax(perturbed_logits, dim = -1)

        return logits, perturbed_logits
...
def model_eval(dataloader, model, device):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        logits, perturbed_logits = model(b_ids, b_mask)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            loss = loss +  0.001 * (F.mse(logits, perturbed_logits, reduction = 'mean'))/args.batch_size
