...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    normalized_attention_scores = nn.Softmax(dim = -1)(attention_scores)
    normalized_attention_scores = self.dropout(normalized_attention_scores)
    # multiply the attention scores to the value and get back V' 
   
    context = torch.matmul(normalized_attention_scores, value)
    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    context = context.permute(0, 2, 1, 3).contiguous()
    context_shape = context.shape
    context = context.view(context_shape[0], context.shape[1], self.attention_head_size * self.num_attention_heads)
    return context
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    dense_layer_output = dropout(dense_layer_output)
    add_norm = ln_layer(input + dense_layer_output)
    return add_norm
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    attention_context = self.self_attention(hidden_states, attention_mask)
    # add-norm layer
    normalized_attention_context = self.add_norm(hidden_states, attention_context, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    # feed forward
    feed_forward = self.interm_dense(normalized_attention_context)
    feed_forward = self.interm_af(feed_forward)
    # another add-norm layer
    normalized_feed_forward = self.add_norm(normalized_attention_context, feed_forward, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def forward(self, input_ids, attention_mask, perturbed_embeddings = None):
    """
    ...
    if perturbed_embeddings is not None:
      embedding_output = perturbed_embeddings
    else:
